...
plotBED(dframe$date, dframe$guestNights2018, xlab = "2018", ylab= "Guest Nights")
# x and y text
...
maxGuestDate <- as.Date(dframe[dframe$guestNights2018==maxGuestNights, "date"])
# captioning the date connected to the highest number 
```

The month with the highest activity is `r format(maxGuestDate, "%B")`. The total number of guest nights is `r format(maxGuestNights, big.mark= " ")`.
...
```{r table}
kable(dframe, row.names = F, align = "c") # Creating a table
```
